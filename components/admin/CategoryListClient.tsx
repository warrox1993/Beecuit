...
export function CategoryListClient({ rows }: { rows: Row[] }) {
  ...
  return (
    <div className="space-y-4">
      {rows.map((r) =>
        editing === r.id ? (
          <div key={r.id} className="border-warm-brown/10 rounded-lg border p-4">
            <CategoryForm
              initial={{
                id: r.id,
                slug: r.slug,
          ...
          >
            <div>
              <p className="text-warm-brown/60 font-mono text-xs">{r.slug}</p>
              <p className="text-warm-brown">{r.translations.fr.name}</p>
              <p className="text-warm-brown/60 text-xs">
                {r.productCount} produit(s) · sort {r.sortOrder} ·{" "}
            ...
            <div className="flex flex-col gap-2">
              ...
              <Button
                ...
                onClick={() => {
                  if (
                    !confirm(
                      `Supprimer "${r.translations.fr.name}" ? Les produits perdront leur catégorie.`,
                    )
                  )
                    return;
                  start(async () => {
