...
export function ProductTranslationTabs({
  ...
  return (
    <div>
      <div className="border-warm-brown/10 flex gap-1 border-b">
        {(["fr", "nl", "de", "en"] as const).map((l) => (
          <button
            key={l}
            type="button"
            onClick={() => setActive(l)}
            className={`relative px-4 py-2 text-sm uppercase ${active === l ? "border-honey text-honey-dark border-b-2" : "text-warm-brown/60"}`}
          >
            {l}
            {incomplete(l) && (
      ...
      <div className="mt-4 space-y-3">
        <label className="block">
          <span className="text-warm-brown text-xs">Nom</span>
          <input
        ...
        <label className="block">
          <span className="text-warm-brown text-xs">Slug (kebab-case)</span>
          <input
        ...
        <label className="block">
          <span className="text-warm-brown text-xs">Description courte (max 160)</span>
          <input
            maxLength={160}
            className="border-warm-brown/20 mt-1 w-full rounded border bg-white px-3 py-2 text-sm"
        ...
        <label className="block">
          <span className="text-warm-brown text-xs">Ingrédients</span>
          <textarea
            rows={3}
            className="border-warm-brown/20 mt-1 w-full rounded border bg-white px-3 py-2 text-sm"
            value={t.ingredients}
        ...
        <fieldset>
          <legend className="text-warm-brown text-xs">Allergènes</legend>
            ...
            {ALLERGENS.map((a) => {
              ...
              return (
                <button
                  ...
                  onClick={() =>
                    set({
                      allergens: on ? t.allergens.filter((x) => x !== a) : [...t.allergens, a],
                    })
                  }
        ...
        <fieldset>
          <legend className="text-warm-brown text-xs">Valeurs nutritionnelles /100 g</legend>
            ...
            {(["energy_kcal", "fat_g", "carbs_g", "protein_g", "salt_g"] as const).map((k) => (
              <label key={k}>
                ...
                <input
                  type="number"
                  step="0.1"
                  value={t.nutritionalFactsPer100g[k]}
                  onChange={(e) =>
                    set({
                      nutritionalFactsPer100g: {
                        ...t.nutritionalFactsPer100g,
                        [k]: Number(e.target.value),
                      },
                    })
                  }
                  className="border-warm-brown/20 mt-1 w-full rounded border bg-white px-2 py-1"
                />
              </label>
            ))}
          </div>
        ...
        <label className="block">
          <span className="text-warm-brown text-xs">SEO title (max 60)</span>
          <input
            maxLength={60}
            className="border-warm-brown/20 mt-1 w-full rounded border bg-white px-3 py-2 text-sm"
            value={t.seoTitle}
