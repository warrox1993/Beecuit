  ...
  return (
    <div className="space-y-3">
      <div className="grid grid-cols-4 gap-2">
        {images.map((img) => (
          <div key={img.id} className={`group relative overflow-hidden rounded border ${img.isPrimary ? "border-honey ring-2 ring-honey" : "border-warm-brown/20"}`}>
            ...
            <div className="absolute inset-x-0 bottom-0 flex gap-1 bg-black/50 p-1 text-xs text-white opacity-0 group-hover:opacity-100">
              <button type="button" onClick={() => start(async () => { await setPrimaryImage(img.id, productId); })} disabled={pending || img.isPrimary}>★</button>
              <button type="button" onClick={() => start(async () => { await deleteImage(img.id, productId); })} disabled={pending}>×</button>
            </div>
          </div>
        ))}
      </div>
