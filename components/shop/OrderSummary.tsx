...
export function OrderSummary({
  ...
  return (
    <aside className="border-warm-brown/10 sticky top-28 rounded-2xl border bg-white p-6 shadow-md">
      <p className="text-honey-dark mb-6 text-xs font-semibold tracking-[0.1em] uppercase">
        RÉCAPITULATIF
      </p>
      <ul className="divide-warm-brown/10 mb-6 divide-y text-sm">
        {lines.map((l, i) => (
          <li key={i} className="flex justify-between py-3">
            ...
            <span className="text-warm-brown font-mono">
              {eur(l.unitPriceCents * l.quantity)} €
            </span>
          </li>
        ))}
      </ul>
      <div className="border-warm-brown/10 space-y-2 border-t pt-4 text-sm">
        <div className="text-warm-brown flex justify-between">
